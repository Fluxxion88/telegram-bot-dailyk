...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not get_user(update.effective_user.id):
        add_user(update.effective_user.id, update.effective_user.first_name )
...
async def add_habit_command(update, context):
    ...
    elif get_habit_by_name(user_id, text_name):
        await context.bot.send_message(
        chat_id=update.effective_chat.id, 
        text="You already have this habit."
        )
    else:
        add_habit(user_id, text_name)
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
...
async def delete_habit_command(update, context):
    ...
    if context.args[0].isdigit():
        ...
        if delete_habit(habit_id, update.effective_user.id):
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
...
async def done_command(update, context):
    ...
    if context.args[0].isdigit():
        habit_id = int(context.args[0])
        if get_habit_by_id(habit_id, user_id):
...
async def streak_command(update, context):
    if not context.args:
        ...
        return 
    if context.args[0].isdigit():
        habit = get_habit_by_id(int(context.args[0]), update.effective_user.id)
        if habit:
            count = count_completions(int(context.args[0]))
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
